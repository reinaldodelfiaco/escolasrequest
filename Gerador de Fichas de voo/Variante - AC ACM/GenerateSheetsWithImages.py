...
def pdf2img(caminhoArquivoPDF):
    ...
    cada página constante no arquivo .pdf original.

    Args:
        caminhoArquivoPDF: string contendo o caminho absoluto para um arquivo .pdf a ser convertido em imagens (.jpeg)
    """
    gs = "C:\\Program Files\\gs\\gs9.50\\bin\\gswin64c.exe"
    command = f'"{gs}" -dBATCH -dNOPAUSE -q -sDEVICE=jpeg -r300 -dAutoRotatePages=/None ' \
              f'-sOutputFile="{caminhoArquivoPDF[:-4]} - %03d.jpeg" "{caminhoArquivoPDF}"'
...
def csvParaFichasDeAvaliacao(arquivoCsv, nomeCiac):
    ...
    for voo in chavesVoos:
        ...
        voos[voo]['Logo'] = img
        voos[voo]['Objectives'] = RichText(voos[voo]['Objectives'])
...
if __name__ == '__main__':
    ...
    for arquivos in diretorio:  # Para todos os arquivos csv no diretório, executa a função csvParaFichasDeAvaliação
        if arquivos[-4:].lower() == '.csv' and '#' not in arquivos:
    ...
    diretorio = os.getcwd()
    conversaoJPG = datetime.datetime.now()
    listaDeArquivos = [f'{diretorio}\\{arquivo}' for arquivo in os.listdir(diretorio)]
    for arquivo in listaDeArquivos:
        if arquivo.lower()[-4:] == '.pdf' and '#' not in arquivo.lower():
            pdf2img(arquivo)

    deltaT = str(datetime.datetime.now() - conversaoJPG).split(':')  # Calcula o tempo necessário para a conversão
    print(f"Conversão para .jpg terminada em {f'{int(deltaT[1])} minuto' if int(deltaT[1]) > 0 else ''}"
          f"{'s' if int(deltaT[1]) > 1 else ''}"
          f"{' e ' if int(deltaT[1]) > 0 and round(float(deltaT[2]), 1) > 0 else ''}"
          f"{f'{round(float(deltaT[2]), 1)} segundo' if float(deltaT[2]) > 0 else ''}"
          f"{'s' if round(float(deltaT[2]), 1) > 1 else ''}")  # Mostra a duração do processo de conversão
